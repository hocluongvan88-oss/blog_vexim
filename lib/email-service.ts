...
export class EmailService {
  ...
  private async sendEmail({
    ...
    try {
      // If RESEND_API_KEY is not set, log email instead of sending (dev mode)
      if (!RESEND_API_KEY) {
  ...
  private async getLatestAlerts(): Promise<FDAItem[]> {
    try {
      ...
      // If no cache, fetch from FDA API (would need to import fdaApi here)
